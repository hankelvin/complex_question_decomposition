...
class RankLLM(ABC):
    ...
    def permutation_pipeline(
        ...
        outputs, out_token_count = self.run_llm(
            prompt, current_window_size = rank_end - rank_start
        )
        if logging:
            print(f"output: {outputs}")
        ranking_exec_info = RankingExecInfo(
            prompt, outputs, in_token_count, out_token_count
        )
        result.ranking_exec_summary.append(ranking_exec_info)
        result = self.receive_permutation(result, outputs, rank_start, rank_end)
        return result
    ...
    def sliding_windows(
        ...
        ### CHANGE START ###
        for key in ['num_hops', 'perm_order']:
            if hasattr(request, key):
                setattr(rerank_result, key, getattr(request, key))
        ### CHANGE END ###
