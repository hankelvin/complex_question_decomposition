...
def main():
    ...
    if args.do_single_hop: model_name += '-singlehop'
    if args.sq_all_paragraphs: model_name += '-sq_all_pararagraphs'
    print(f'📘📘 do_single_hop: {args.do_single_hop}')
    args.output_dir = os.path.join(args.output_dir, date_curr, model_name)
    ### CHANGE START ###
    args, start_epoch, global_step, batch_step, \
        best_f1, train_loss_meter_vals = \
            give_init_vals(args)
    ### CHANGE END ###
    ...
    model = Reader(bert_config, args.model_name, task_type = args.dataset_type, 
                   len_tokenizer = len(tokenizer) if args.dataset_type == 'hotpot' else 0, 
                   gradient_checkpointing=args.gradient_checkpointing)
    
    ### CHANGE START ###
    eval_dataset = MHReaderDataset(tokenizer, args.predict_file, max_len = args.max_seq_len, 
                                   ### CHANGE START ###
                                   dataset_type = args.dataset_type, 
                                   ### CHANGE END ###
                                   is_train = False, do_single_hop = args.do_single_hop)
    ### CHANGE END ###
    ...
    if args.init_checkpoint != "":
        ...
        ### CHANGE START ###
        print(f'📘📘Resuming from epoch {start_epoch} checkpoint:', args.init_checkpoint)
        ### CHANGE END ###
    ...
    if args.do_train:
        ### CHANGE START ### allow for resuming training
        ...
        ### CHANGE START ###
        if train_loss_meter_vals is not None:
            for k,v in train_loss_meter_vals.items():
                setattr(train_loss_meter, k, v)
        ### CHANGE END ###
        model.train()

        ### CHANGE START ###
        train_dataset = MHReaderDataset(tokenizer, args.train_file, max_len = args.max_seq_len, 
                                        ### CHANGE START ###
                                        dataset_type = args.dataset_type, 
                                        ### CHANGE END ###
                                        is_train = True, do_single_hop = args.do_single_hop)
        ### CHANGE END ###
        if args.local_rank != -1:
        ...
        ### CHANGE START ###
        if global_step >= warmup_steps: warmup_steps = 0
        ### CHANGE END ###
        ...
        for epoch in range(start_epoch, int(args.num_train_epochs)):
            ### CHANGE START
            for bn, batch in enumerate(tqdm(train_dataloader)):
                ### CHANGE END
                batch_step += 1
                id = batch.pop('id')
            ...
            if args.local_rank == -1 or args.local_rank == 0:
                ...
                ### CHANGE START ###
                with open(os.path.join(args.output_dir, f"checkpoint_last_num.json"), 'w+') as f:
                    out_line = {'epoch': epoch+1, 'global_step': global_step, 
                                'batch_step': batch_step, 'best_f1': best_f1}
                    train_loss_meter_vals = {key: getattr(train_loss_meter, key)
                                             for key in ['val', 'avg', 'sum', 'count']}
                    out_line[f'train_loss_meter_vals'] = train_loss_meter_vals
...
### CHANGE START ###
def give_init_vals(args):
    if args. init_checkpoint != "":
        args.output_dir = dp = os.path.dirname(args.init_checkpoint)
        with open(os.path.join(dp, f"checkpoint_last_num.json"), 'r') as f: 
            out_line = json.load(f)
            print('📘📘 OUTLINE', out_line)
            start_epoch = out_line['epoch']
            global_step = out_line['global_step']
            batch_step  = out_line['batch_step']
            best_f1     = out_line['best_f1']
            train_loss_meter_vals = out_line[f'train_loss_meter_vals']
    else: 
        start_epoch = 0 
        global_step = 0  # gradient update step
        batch_step = 0  # forward batch count
        best_f1 = 0
        train_loss_meter_vals = None
    
    return args, start_epoch, global_step, batch_step, \
        best_f1, train_loss_meter_vals
### CHANGE END ###
